...
2. **`LEVELS`** — array of 18 circuit definitions. Each has plain-object `spawn:{x,y}`, `goal:{x,y,w,h}`, `walls:[{x,y,w,h},...]` in virtual units, an `ink` multiplier, and `medals: { platinum, gold, silver, bronze }` in ms. Levels do **not** depend on screen size.
3. **Persistent storage** — keys: `timeball.bestTimes.v3`, `timeball.ghosts.v4`, `timeball.settings.v3`, `timeball.progress.v3`, `timeball.plays.v3`. Bump the version suffix on schema changes. (Ghosts went v3→v4 when coords moved from W/H-normalized to virtual.)
